...
def test_rank_colours_and_transient_culls_are_explained_in_the_viewer():
    ...
    assert 'Show past culled rays' in source
...
class _FakeProcess:
    def __init__(self, *args, **kwargs):
        self.stdin = _FakeStdin(); self.returncode = None
...
def test_process_manager_reuses_one_child_and_sends_stdin_json(tmp_path):
    ...
    def factory(*args, **kwargs):
        child = _FakeProcess(*args, **kwargs); children.append(child); return child
    manager = RayReviewProcess(root, process_factory=factory)

    manager.open('one'); manager.open('two')
