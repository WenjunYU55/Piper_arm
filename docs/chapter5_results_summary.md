...
The evidence was generated on 31 August 2026 from commit `6bd89db`, with the
recorded worktree state explicitly marked dirty because the current
reconstruction branch also contains the reviewed 3-degree constrained
superposition change. No robot, CAN interface, camera, ROS graph, or GPU model
was started for this analysis. The only new computation was command-free replay
of saved RGB-D artifacts, ray diagnostics, capability-map benchmarks, and
...
approximation produced zero state-level false negatives in that sample and 501
...
Tesseract is algorithmically faster or globally superior to cuRobo.
...
noise, rejected-observation admission, matched Tesseract-versus-cuRobo planning,
...
| Motion planning | Tesseract; exploratory cuRobo | Physical qualification and collision replay | Tesseract for final hardware system | Physically integrated; cuRobo remains unqualified | Strongest validated integration evidence |
...
5. **Cannot conclude:** map support is not exact IK, and no matched planner
   benchmark shows Tesseract algorithmically superior to cuRobo.
