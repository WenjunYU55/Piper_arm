...
def canonical_ray_universe(viewpoints):
    """Return the mission-stable ray identities, independent of bounds."""
    records = []
    for order, candidate in enumerate(viewpoints, start=1):
        ray_id = int(candidate.get('ray_id', candidate.get('index', -1)))
        if ray_id < 0:
        ...
        records.append({
            'ray_id': ray_id,
            'generated_order': int(candidate.get('generated_order', order)),
            # A standoff-only envelope update may reproduce the same unit
            # vector through one extra multiply/subtract/divide round trip.
            # Nanoradian-scale quantization keeps that numerical noise out of
            # mission identity while remaining far below any planner angle.
            'direction': [round(value / norm, 9) for value in direction],
        })
    records.sort(key=lambda item: item['ray_id'])
    return records
...
def ray_universe_sha256(viewpoints, target_center, frame_id):
    """Bind immutable directions to their frozen centre and planning frame."""
    return population_sha256({
        'frame_id': str(frame_id),
        'rays': canonical_ray_universe(viewpoints),
...
def ray_population_identity(
    ...
    records = canonical_ray_universe(viewpoints)
    return {
        'schema_version': PROTOCOL_VERSION,
        'mission_id': str(mission_id),
        'session_id': str(session_id),
        'sha256': ray_universe_sha256(
            viewpoints, target_center, frame_id),
