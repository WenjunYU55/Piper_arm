...
class TargetCloudNode(Node):
    ...
    def frame_cb(self, color_msg, depth_msg, camera_info):
        ...
        try:
            ...
            return
        if self.latest_mask.shape != depth.shape[:2] or color.shape[:2] != depth.shape[:2]:
        ...
        self.publish_status('accumulating', frame=frame, frame_points=int(u.size), voxel_count=len(self.voxels))
