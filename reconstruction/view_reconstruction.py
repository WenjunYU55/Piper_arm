...
def expected_wireframe(report, mesh):
    ...
    if dimensions.shape != (3,):
        return None
    observed = mesh.get_oriented_bounding_box(robust=True)
...
def main():
    ...
    geometries = [mesh, o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.05)]
    wireframe = expected_wireframe(report, mesh)
    ...
    if args.show_input:
        cloud_path = Path(str(report.get('input_cloud_path', ''))).resolve()
