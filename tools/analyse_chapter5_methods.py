...
def analyse_capture_geometry():
    ...
    rows = []
    pattern = REPOSITORY / 'datasets' / 'active_scan' / 'scan_*' / \
    ...
    for metadata_path in sorted(glob.glob(str(pattern))):
        ...
        rows.append({
            ...
            'source_metadata': str(metadata_path.relative_to(REPOSITORY)),
        })
    return rows
...
def analyse_scan_runs():
    ...
    for scan_dir in sorted((
            REPOSITORY / 'datasets' / 'active_scan').glob('scan_*')):
...
def analyse_offline_resegmentation():
    ...
    for row in value['frames']:
        rows.append({
            ...
            'source_index': str(path.relative_to(REPOSITORY)),
...
def analyse_feasibility_events():
    ...
    root = REPOSITORY / 'datasets' / 'ray_diagnostics'
    rows = []
...
def documented_evidence():
    ...
    return {
        ...
        'curobo_collision_replay': {
            ...
            'mutually_clear': 1456,
            'curobo_conservative_false_positives': 501,
            'curobo_state_level_false_negatives': 0,
            'source': (
                'git curobo-integration:docs/architecture/'
                'motion_planner_backends.md at cca6844'),
            'comparison_strength': 'CONTROLLED_REPLAY',
            'limitation': (
...
def build_comparisons(data):
    ...
    rows.append(comparison_row(
        ...
        'sample but rejected 501 exact-model-clear states.',
        curobo['limitation'],
...
def build_final_selection(data):
    ...
    return [
        ...
        },
        {
            ...
            'Evidence': (
                'Tesseract is physically integrated; cuRobo had 0/2004 '
                'state-level false negatives but 501 conservative false '
                'positives and remains hardware_qualified=false.'),
            'Reason Retained': (
...
def unsupported_comparisons():
    ...
    return [
        ...
        {
            'comparison': 'Tesseract vs cuRobo planning time/success',
            'reason': (
                'No same-request backend replay exists and cuRobo has not been '
                'physically qualified.'),
            'required_data': (
                'identical frozen requests, matched collision models, offline '
                'timings, and separately labelled physical qualification'),
        },
        {
    ...
    planner = [
        {
            ...
            'comparison_strength': 'HISTORICAL_OBSERVATIONAL',
            'limitation': 'No matched backend timing comparison.',
        },
        {
...
def git_state():
    ...
    head = subprocess.check_output(
        ['git', 'rev-parse', 'HEAD'], cwd=str(REPOSITORY), text=True).strip()
    branch = subprocess.check_output(
        ['git', 'branch', '--show-current'], cwd=str(REPOSITORY),
        text=True).strip()
    status = subprocess.check_output(
        ['git', 'status', '--short'], cwd=str(REPOSITORY), text=True)
    return {'head': head, 'branch': branch, 'worktree_dirty': bool(status.strip())}
...
def main():
    ...
    data = {
        ...
        'analysis_scope': (
            ...
            'methods using repository evidence available on 2026-08-31.'),
        'git': git_state(),
